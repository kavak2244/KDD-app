...
export default function App() {
  ...
  const [hardwareLogs, setHardwareLogs] = useState([
    ...
    },
    {
      ...
      type: 'at',
      direction: 'tx',
      payload: 'AT+CSQ => +CSQ: 28,99 (Signal Excellent)',
      description: 'استعلام سطح سیگنال ماژول SIM800',
    },
  ]);
  ...
  const [events, setEvents] = useState([
    {
      ...
      message: 'توسط کاربر از طریق اپلیکیشن (اینترنت ابری)',
      level: 'info',
    },
    {
      time: '۰۹:۱۵',
      title: 'روشنایی کانتینر روشن شد',
      message: 'خروجی رله ۳ با موفقیت تحریک گردید',
      level: 'info',
    },
    {
      time: 'دیروز ۱۸:۳۰',
      title: 'هشدار تست باتری',
  ...
  const handleToggleArm = () => {
    ...
    if (nextState) {
      ...
      setEvents((prev) => [
        { time: 'همین الان', title: 'سیستم مسلح شد (ARM)', message: `سیستم امنیتی توسط کاربر فعال گردید (${connectionMode === 'sms' ? 'پیامک' : 'اینترنت'})`, level: 'info' },
        ...prev,
      ]);
    } else {
      playSound('disarm');
  ...
  // 2. Trigger Alarm Simulation
  const handleTriggerAlarm = () => {
    playSound('alarm');
    setIsAlarmActive(true);
    logCommand(
      'sms',
      `INCOMING ALARM SMS FROM SIM800: !ALARM! ZONE 1 TRIGGERED (CABIN SENSOR)`,
      'هشدار فوری تحریک سنسور کابین توسط SIM800'
    );
    setEvents((prev) => [
      { time: 'همین الان', title: '🚨 آلارم اضطراری خطر!', message: 'سنسور چشمی داخل کابین تحریک شد!', level: 'alarm' },
      ...prev,
    ]);
  };
  ...
  const handleResetAlarm = () => {
    ...
    logCommand(
      connectionMode === 'sms' ? 'sms' : 'mqtt',
      connectionMode === 'sms' ? `SMS TO ${activeDevice.simNumber}: #SILENCE*1234#` : `MQTT TOPIC: kdd/${activeDevice.serial}/cmd -> {"action":"SILENCE"}`,
      'فرمان قطع صدای آژیر و رفع وضعیت خطر'
    );
  };
  ...
  // 8. Trigger Sensor Test Simulation
  const handleTriggerSensor = (sensorId) => {
    ...
    setEvents((prev) => [
      { time: 'همین الان', title: `تحریک سنسور شماره ${sensorId}`, message: 'هشدار امنیتی شبیه‌سازی شد', level: 'alarm' },
      ...prev,
    ]);
  ...
  // 13. Registration Complete Flow
  const handleCompleteRegistration = () => {
    const newDev = {
      id: `kdd-custom-${Date.now()}`,
      name: regForm.name || 'کامیون جدید KDD',
      model: 'Custom Heavy Vehicle',
      serial: regForm.serial || 'KDD-NEW-9900',
      simNumber: regForm.simNumber || '09120000000',
    ...
    logCommand(
      'sms',
      `SMS: #REG_DEV=${newDev.serial}*PASS=${regForm.password || '1234'}#`,
      'ثبت نهایی دستگاه جدید و فعال‌سازی سیم‌کارت در سرور KDD'
    );
  };
    ...
    <div className="min-h-screen bg-[#06090E] text-gray-100 flex flex-col font-['Vazirmatn',sans-serif] selection:bg-[#1F6BFF] selection:text-white" dir="rtl">
      ...
      <header className="bg-[#0D1117] border-b border-[#2D333B] px-4 py-2.5 flex flex-wrap items-center justify-between gap-3 sticky top-0 z-40 shadow-xl">
        ...
        <div className="flex items-center gap-3">
          <div className="w-8 h-8 rounded-xl bg-gradient-to-br from-[#1F6BFF] to-blue-800 flex items-center justify-center text-white shadow-[0_0_15px_rgba(31,107,255,0.5)]">
          ...
          <div>
            <div className="flex items-center gap-2">
              ...
              <span className="text-[10px] px-2 py-0.2 rounded-full bg-[#1F6BFF]/20 text-[#1F6BFF] font-mono border border-[#1F6BFF]/40">
                پروتوتایپ تعاملی v1.2
              </span>
            </div>
            <p className="text-[11px] text-gray-400">سیستم حفاظتی خودروهای سنگین با کنترل پیامکی SIM800 و اینترنت ابری</p>
          </div>
        </div>
        ...
        <div className="flex items-center gap-2">
          ...
          <div className="flex bg-[#161B22] rounded-xl p-1 border border-[#2D333B] text-xs">
            ...
            >
              ...
              <span>مشخصات و صفحات ۱۴گانه</span>
            </button>
          </div>
        </div>
      </header>
      ...
      <main className="flex-1 flex flex-col items-center justify-center p-2 sm:p-6 relative overflow-x-hidden">
        {viewMode === 'spec' ? (
          ...
          <div className="relative my-auto">
            {/* Phone Outer Chassis */}
            <div className={`relative bg-[#000000] border-[6px] border-[#2C313C] rounded-[48px] shadow-[0_25px_60px_-15px_rgba(0,0,0,0.9),0_0_40px_rgba(31,107,255,0.15)] overflow-hidden transition-all duration-300 ${
              isFullScreenPhone ? 'w-full max-w-md h-[92vh]' : 'w-[375px] sm:w-[390px] h-[780px]'
            }`}>
              ...
              <div className="w-full h-[calc(100%-70px)] bg-[#0D1117] overflow-y-auto flex flex-col relative">
                {activeScreen === 'splash' && (
                  <Screen01_Splash
                    onStart={() => setActiveScreen('dashboard')}
                    onRegister={() => setActiveScreen('register-1')}
                  />
                )}
